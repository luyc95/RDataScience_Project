...
## Data
1. Texas boundary shapefile

The shp file was downloaded from [Census Bureau's MAF/TIGER database](https://www.census.gov/geo/maps-data/data/cbf/cbf_cousub.html) and loaded by function readOGR.
...
Data ranged from Janaury 5 2010 to November 28 2017, including dates and drought classification results. The classification system and its meaning are shown as below.
...
Unluckily, the drought index for each county by state can be downloaded just in single day, therefore, I choosed the data released in October 31 as sample. 
...
```
    ...
    + Convert character type into date format and convert the table into xts format
...
Drought sptail distribution maps are shown as below.
...
dygraph(data = time,
  ...
  frameWidget(height =500)
```
Compared with temperature changing line, precipitation's trend line flutuates more dramatically, so that the long term precipitation condition is not very stable. Hence, it is more likely to be dry without steady raining.
...
total%>%
  ...
  geom_line(aes(y=mean.temp),col="red")+
   scale_color_discrete(name = "Y series", labels = c("PDSI", "Precipitation","temperature"))
